...
void command_init() {
	...
	mbedtls_pk_init(&ctx);
	mbedtls_pk_parse_public_key(&ctx, (const unsigned char*) pem, strlen(pem) + 1);
}
...
void command_receive(const lownet_frame_t* frame) {
	...
	switch(type) {
		...
		case SIGNED:
			...
			mbedtls_sha256((const unsigned char*) frame, sizeof(lownet_frame_t), msg_hash, 0);
			state = WAIT_SIG;
			break;
		case SIG1:
			if (state != WAIT_SIG && state != WAIT_SIG1) return;

			const cmd_signature_t* sig1 = (const cmd_signature_t*) &frame->payload;
			if (memcmp(msg_hash, sig1->hash_msg, CMD_HASH_SIZE) != 0) return;
			if (memcmp(key_hash, sig1->hash_key, CMD_HASH_SIZE) != 0) return;

			memcpy(&buffer[1], frame, sizeof(lownet_frame_t));
			state = (state == WAIT_SIG) ? WAIT_SIG2 : READY;
			break;
		case SIG2:
			if (state != WAIT_SIG && state != WAIT_SIG2) return;

			const cmd_signature_t* sig2 = (const cmd_signature_t*) &frame->payload;
			if (memcmp(msg_hash, sig2->hash_msg, CMD_HASH_SIZE) != 0) return;
			if (memcmp(key_hash, sig2->hash_key, CMD_HASH_SIZE) != 0) return;
	...
	uint8_t expected[CMD_BLOCK_SIZE];
	uint8_t actual[CMD_BLOCK_SIZE];
	memset(expected, 0, 220);
	memset(expected + 220, 1, 4);
	memcpy(expected + 224, msg_hash, CMD_HASH_SIZE);

	uint8_t signature[CMD_BLOCK_SIZE];
	memcpy(signature, ((cmd_signature_t*) buffer[2].payload)->sig_part, CMD_BLOCK_SIZE / 2);
